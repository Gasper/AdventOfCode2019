...
fn main() {
    ...
    let closest_intersect = closest_intersect(wire1, wire2);
...
impl Line {
    fn intersection(&self, other: Line) -> Result<i64, String> {
        ...
        if self.direction != other.direction    &&
            ...
            // For each wire, this is total distance + distance to intersection
            let path_vert: i64 = vertical.distance + (vertical.y1 - horizontal.y1).abs();
            let path_hor: i64 = horizontal.distance + (horizontal.x1 - vertical.x1).abs(); 
            return Ok(path_hor + path_vert);
        }
        else {
...
fn parse_wire(path: String) -> Wire {
    ...
    for segment in path.split(',') {
        ...
        match direction {
            "U" => {
                y += distance;
                segments.push(Line {x1: x1, y1: y1, x2: x, y2: y, 
                    distance: segment_start, direction: WireDirection::Vertical});
            },
            "D" => {
                y -= distance;
                segments.push(Line {x1: x1, y1: y1, x2: x, y2: y, 
            ...
            "L" => {
                x -= distance;
                segments.push(Line {x1: x1, y1: y1, x2: x, y2: y, 
                    distance: segment_start, direction: WireDirection::Horizontal});
            }
            "R" => {
                x += distance;
                segments.push(Line {x1: x1, y1: y1, x2: x, y2: y, 
...
fn closest_intersect(wire1: Wire, wire2: Wire) -> i64 {

    let mut intersects: Vec<i64> = wire1.segments.into_iter()
        .cartesian_product(wire2.segments)
...
mod test {
    ...
    fn test_intersection() -> Result<(), String> {
        let line1 = Line {x1: 0, y1: 0, x2: 0, y2: 8, distance: 3, direction: WireDirection::Vertical};
        let line2 = Line {x1: 0, y1: 0, x2: 8, y2: 0, distance: 1, direction: WireDirection::Horizontal};
        assert_eq!(line1.intersection(line2)?, 4);

        // The same as previous, but direction of one line is reversed
        let line1 = Line {x1: 0, y1: 0, x2: 0, y2: 8, distance: 1, direction: WireDirection::Vertical};
        let line2 = Line {x1: 8, y1: 0, x2: 0, y2: 0, distance: 5, direction: WireDirection::Horizontal};
        assert_eq!(line1.intersection(line2)?, 14);

        // Example 1 from aoc
        let line1 = Line {x1: 3, y1: 2, x2: 3, y2: 6, distance: 0, direction: WireDirection::Vertical};
        let line2 = Line {x1: 2, y1: 3, x2: 6, y2: 3, distance: 1, direction: WireDirection::Horizontal};
        assert_eq!(line1.intersection(line2)?, 3);

        // Example 2 from aoc
        let line1 = Line {x1: 6, y1: 3, x2: 6, y2: 7, distance: 2, direction: WireDirection::Vertical};
        let line2 = Line {x1: 3, y1: 5, x2: 8, y2: 5, distance: 1, direction: WireDirection::Horizontal};
        assert_eq!(line1.intersection(line2)?, 8);

        // Debug example 1
        let line1 = Line { x1: 6, y1: 3, x2: 2, y2: 3, distance: 1, direction: WireDirection::Horizontal };
        let line2 = Line { x1: 3, y1: 5, x2: 3, y2: 2, distance: 1, direction: WireDirection::Vertical };
        assert_eq!(line2.intersection(line1)?, 7);
    ...
    fn test_wire_parsing() {
        ...
        assert_eq!(wire.segments[0].x1, 0);
        assert_eq!(wire.segments[0].y1, 0);
        assert_eq!(wire.segments[0].x2, 3);
        assert_eq!(wire.segments[0].y2, 0);
        assert_eq!(wire.segments[0].distance, 0);

        assert_eq!(wire.segments[1].x1, 3);
        assert_eq!(wire.segments[1].y1, 0);
        assert_eq!(wire.segments[1].x2, 3);
        assert_eq!(wire.segments[1].y2, 3);
        assert_eq!(wire.segments[1].distance, 3);

        assert_eq!(wire.segments[2].x1, 3);
        assert_eq!(wire.segments[2].y1, 3);
        assert_eq!(wire.segments[2].x2, 1);
        assert_eq!(wire.segments[2].y2, 3);
        assert_eq!(wire.segments[2].distance, 6);

        assert_eq!(wire.segments[3].x1, 1);
        assert_eq!(wire.segments[3].y1, 3);
        assert_eq!(wire.segments[3].x2, 1);
        assert_eq!(wire.segments[3].y2, 2);
        assert_eq!(wire.segments[3].distance, 8);
    }
    ...
    fn test_example1() {
        ...
        assert_eq!(closest_intersect(wire1, wire2), 610);
    }
    ...
    fn test_example2() {
        ...
        assert_eq!(closest_intersect(wire1, wire2), 410);
    }
}